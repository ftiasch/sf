...
Theorem mult_comm : forall m n : nat,
 m * n = n * m.
Proof.
  assert (H : forall n m : nat, n + n * m = n * (S m)).
  Case "Proof of assertion".
    intros n m.
    induction n as [| n'].
    SCase "n = O".
...
Theorem binary_inverse :
  forall n : nat, n = bin_to_nat (nat_to_bin n).
Proof.
  induction n as [| n'].
  Case "n = O".
    reflexivity.
...
Fixpoint normalize (b : bin) :=
  match b with
    ...
    | one_more_than_twice b' => one_more_than_twice (normalize b')
  end.

Theorem binary_inverse' :
  forall b : bin, normalize b = nat_to_bin (bin_to_nat b).
Proof.
  admit. (* FIXME *)
Qed.
